...
def read_data_from_excel_upstairs(filepath):
    #iterates through excel file and extracts all plate data in rows, but for upstairs plate reader format
    data = pd.read_excel(filepath, 0, header=None, engine="calamine")
    all_data = {}
    for row_num, row in enumerate(data.values):
        if "Name" in str(row[0]):
            data_name = row[1] #finds label
            #print(data_name)
        if "Start Time" in str(row[0]):
            data_time = row[4] #finds time stamp
        if str(row[0]) in key_rows:
            if data_name not in all_data.keys():
                all_data[data_name] = [[data_time, row[0], row[1:13]]] #extracting all data into that label
            else:
...
data_folder = "26-06-05_kirill_single_plasmid_2"
data_files = ["26-06-05_kirills_single_plasmid_t0.xlsx",
             "26-06-05_kirills_single_plasmid_t3.xlsx",
             "26-06-05_kirills_single_plasmid_t6.xlsx",
             "26-06-05_kirills_single_plasmid_t8.xlsx",
             ]

save_filename_starter = "26-06-05_kirill_single_plasmid_2"
